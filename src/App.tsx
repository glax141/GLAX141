...
function AdminPanel({ onClose }: { onClose: () => void }) {
  ...
  const handleSyncToGist = async () => {
    if (!gistToken.trim()) {
      setSyncStatus('❌ 请先在上方输入 GitHub Token 并点击「保存 Token」');
      return;
    }
    ...
    try {
      ...
      console.log('📤 同步数据大小:', (jsonStr.length / 1024).toFixed(2), 'KB');
      ...
      setSyncStatus('✅ 同步成功！其他设备刷新页面即可看到更新');
      ...
      if (error.message.includes('Failed to fetch')) {
        errorMsg = '网络错误：无法连接到 GitHub，请检查网络或 VPN';
      } else if (error.message.includes('401')) {
        errorMsg = 'Token 无效：请检查 Token 是否正确';
      } else if (error.message.includes('403')) {
        errorMsg = '权限不足：Token 可能没有 gist 权限';
      } else if (error.message.includes('413')) {
        errorMsg = '数据太大：请减少图片数量或使用图片链接';
      }
      
      setSyncStatus(`❌ 同步失败: ${errorMsg}`);
  ...
  const handleLoadFromGist = async () => {
    if (!gistToken.trim() || !gistId) {
      setSyncStatus('❌ 请先保存 Token 并确保已有同步数据');
      return;
    }
    ...
    try {
      ...
      // 同时保存到 localStorage
      localStorage.setItem('site_about', JSON.stringify(data.about));
      localStorage.setItem('site_home', JSON.stringify(data.home));
      localStorage.setItem('site_projects', JSON.stringify(data.projects));

      setSyncStatus('✅ 加载成功！页面已更新');
      setTimeout(() => setSyncStatus(''), 3000);
      
      // 提示用户刷新页面查看效果
      setTimeout(() => {
        setSyncStatus('💡 提示：请刷新整个页面查看最新效果');
      }, 3500);
    } catch (error: any) {
      ...
      if (error.message.includes('Failed to fetch')) {
        errorMsg = '网络错误：无法连接到 GitHub';
      } else if (error.message.includes('404')) {
        errorMsg = 'Gist 不存在或已被删除';
      } else if (error.message.includes('401')) {
        errorMsg = 'Token 无效';
      }
      
      setSyncStatus(`❌ 加载失败: ${errorMsg}`);
      setTimeout(() => setSyncStatus(''), 5000);
    } finally {
      setSyncing(false);
    }
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black overflow-y-auto">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 space-y-12">
        {/* 🚨 使用指南 */}
        <section className="border-2 border-yellow-500/40 rounded-xl p-6 bg-yellow-500/5">
          <h3 className="text-yellow-400 text-lg font-medium mb-4 tracking-wider flex items-center gap-2">
            ⚠️ 重要使用指南
          </h3>
          <div className="text-white/80 text-sm space-y-2">
            <p className="font-bold">📌 第一次使用请按顺序操作：</p>
            <ol className="list-decimal list-inside space-y-1 text-white/60 ml-4">
              <li>在下方输入 GitHub Token</li>
              <li>点击「保存 Token」按钮</li>
              <li>修改内容后，点击「同步到云端」</li>
              <li>其他设备刷新页面即可看到更新</li>
            </ol>
            <p className="text-yellow-400/80 text-xs mt-3">
              💡 提示：如果同步失败，请按 F12 打开控制台查看具体错误信息
            </p>
          </div>
        </section>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-white/60 text-sm mb-2">
                步骤1️⃣ 输入 GitHub Token
                {gistToken && <span className="text-green-400 ml-2">✓ 已输入</span>}
              </label>
              <div className="flex gap-3">
                <input
                  ...
                  className="flex-1 bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-white text-sm focus:outline-none focus:border-blue-500/50"
                />
                <MagnetButton
                  onClick={handleSaveToken}
                  className="px-6 py-3 bg-green-500/20 hover:bg-green-500/30 text-green-400 rounded-lg text-sm transition-all whitespace-nowrap"
                >
                  保存 Token ✓
                </MagnetButton>
              </div>
            ...
            {gistId ? (
              <div className="bg-green-500/10 border border-green-500/30 rounded-lg p-3">
                <label className="block text-green-400 text-sm mb-1">✅ 已连接 Gist</label>
                <input
                  type="text"
                  value={gistId}
                  readOnly
                  className="w-full bg-transparent border-none text-white/50 text-xs"
                />
              </div>
            ) : (
              <div className="bg-white/5 border border-white/10 rounded-lg p-3">
                <label className="block text-white/40 text-sm mb-1">⏳ 等待配置...</label>
                <p className="text-white/30 text-xs">保存 Token 后首次同步会自动创建 Gist</p>
              </div>
            )}
            ...
            <div className="pt-4 border-t border-white/10">
              ...
              <div className="flex gap-3">
                <MagnetButton
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-6 py-4 bg-blue-500/30 hover:bg-blue-500/40 text-blue-300 rounded-lg text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                >
                  <I.Cloud />
                  {syncing ? '⏳ 同步中...' : '☁️ 同步到云端'}
                </MagnetButton>
                <MagnetButton
                  ...
                  className="flex items-center gap-2 px-6 py-4 bg-white/10 hover:bg-white/20 text-white rounded-lg text-sm transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <I.Refresh />
                  加载
                </MagnetButton>
          ...
          <div className="max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-16 items-center">
              <div className="relative aspect-[3/4] rounded-lg overflow-hidden">
                <img src={dynamicData.about.avatar} alt="GLAX" className="w-full h-full object-cover" />
              </div>
              <div>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <a
                      ...
                      className="text-white hover:text-white/70 transition-colors"
                    >
                      {dynamicData.about.instagram}
                    </a>
                  </div>
                  <div>
                    <div className="text-white/40 text-xs tracking-wider uppercase mb-2">WeChat</div>
                    <div className="text-white">{dynamicData.about.wechat}</div>
                  </div>
                </div>
              </div>
            </div>
